...
This document defines the intended v0.6.0 release shape: overlaying an annotated
...
v0.6.0 is an **external data sources & manual scales** release: overlay a second
...
Multiple-CSV-in-one-chart introduces the source-declaration grammar that the
following data-backends release (v0.7) will extend. The `Table name = <source>`
declaration (Must item 1) is deliberately designed to also host v0.7's source
constructors later (`Table sales = Sqlite("sales.db", "SELECT ...")`), so the two
releases share one source-declaration grammar rather than inventing two: this
release ships the CSV form and reserves the seam; v0.7 fills it in.
...
- Grammar adds `TableDecl ::= "Table" Ident "=" String` at chart scope; `Table`
  ...
  string-literal CSV path — explicitly leaving room for v0.7 source constructors.
...
- `Cargo.toml` workspace version bumped to `0.6.0` (and `editors/vscode/package.json`)
...
v0.7's source constructors do not exist yet when this lands, so reserve the
source-expression shape in the `Table` value position now; v0.7 will extend it to
`Table x = Sqlite(...)` and other source forms.
...
- Embedded SQL, additional file formats, the Polars backend, and network/command
  sources — now planned for v0.7 (see [`V0_7_PLAN.md`](V0_7_PLAN.md)).
- Everything still deferred from v0.5 (nested spaces, user-defined functions and
  macros) and the standing deferred list in [`V0_3_PLAN.md`](V0_3_PLAN.md).
...
- Tapered-ribbon line width.
- `Table` source-expression generalization toward v0.7 constructors.

### Keep Deferred

- Joins between tables, geographic projection, tapered ribbons (unless promoted).
- All standing-deferred items from v0.5 not promoted here.
