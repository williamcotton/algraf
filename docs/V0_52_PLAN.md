...
- The workflow MUST NOT call `actions/configure-pages` during the build because
  that action fails before first deployment when the repository Pages site has
  not yet been enabled. First-run repository Pages enablement remains a GitHub
  repository setting.
- The workflow MUST compute `/` for `owner.github.io` repositories and
  `/<repo>/` for ordinary project repositories.
- The README and demo README MUST state the first-run GitHub Pages setting and
