...
This document defines the intended v0.7.0 release shape: broadening the data
...
v0.7.0 is a **data backends** release: get data into Algraf from more than one
...
is needed to name a data source. Where v0.6 introduced the `Table name = <source>`
declaration with a CSV-only source expression, this release fills in that seam
with real source constructors (`Table sales = Sqlite(...)`).
...
- `Cargo.toml` workspace version bumped to `0.7.0` when the release branch is ready.
